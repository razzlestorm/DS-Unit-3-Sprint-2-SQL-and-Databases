...
- In the Northwind database, what is the type of relationship between the
  `Employee` and `Territory` tables?
    ...
    -"Employee" and "Territory" keys combine to create the "EmployeeTerritories"
      table. They both seem to be "top-level" tables with no other direct relation.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
      -A document store is appropriate where you want to be able to see multiple
      colllections of values all at once in a single view. And when you want to
      have keys and values that aren't just serialized IDs.  Another example is
      when you want multiple types and formats of data in your collections.
      -When it is not appropriate is when you have ACID requirements, when it's
      actually better that you have consistency, and when you need to ensure
      that each transaction in the database either succeeds or fails.

- What is "NewSQL", and what is it trying to achieve?
    -It is essentially trying to combine NoSQL advantages with ACID-compliant,
    traditional database systems. It is a hybrid of the two, and has the scalability
    of a NoSQL database, with the ACID-consistent relationship model.
